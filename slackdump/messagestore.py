...
class SQLiteStore(object):
    ...
    def __init__(self, db_name="slackdb", table_name="messages", log=DUMMY_LOG):
        ...
        self.db.execute("CREATE TABLE if not exists '%s' (key text, message text)"
                        % (self.table_name))
    ...
    def insert_msg(self, record):
        try:
            ...
            self.db.execute("INSERT INTO {t} VALUES (?, ?)".format(t=t_name),
                            (record['key'], srecord))

            self.log.info("inserted_into_sqlite", key=record['key'],
...
class NsqStore(object):
    ...
    def insert_msg(self, record):
        try:
            ...
            self.log.info("inserted_into_nsq", key=record['key'],
                           channel=record['channel_name'],
                           user=record['user_name'],
                           ts=float(record['ts']))
        except KeyError:
